...
function get_avatar( $id_or_email, $size = '96', $default = '', $alt = false ) {
	...
	if ( is_numeric($id_or_email) ) {
		$UserID = (int) $id_or_email; if($UserID != 0) { $UserID = \Model\UserOperationsModel::UserIDFromWordPressUserID($UserID); };
	} elseif ( is_object($id_or_email) ) {
		$UserID = (int) $id_or_email->user_id; if($UserID != 0) { $UserID = \Model\UserOperationsModel::UserIDFromWordPressUserID($UserID); };
	} else {
		$UserID = (int) \Model\UserOperationsModel::IDFromEmail($id_or_email);
	}
	
	if($UserID == 0) { $UserID = null; }

	$avatar = "<img alt='{$safe_alt}' src='" . \Model\UserOperationsModel::getUserAvatarLinkFromUserID($UserID) . "' class='avatar avatar-{$size} photo' height='{$size}' width='{$size}' />";
