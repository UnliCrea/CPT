...
if ($submit || $preview || $refresh)
{
	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			if($submit && $mode=='quote' && $post_data['quote_username'] != '') {
				define('IN_PHPBB', true);
				require(dirname(__FILE__) . '/../index.php');
				$MyRedirectURL = explode('?', $redirect_url);
				$MyRedirectURL = $MyRedirectURL[1];
				\Model\UserOperationsModel::notifyUserForCommunityQuotation($post_data['quote_username'], $MyRedirectURL, $post_data['topic_title']);
			}
