;(function( $ ){
	...
	var defaults = {  
	   borderColor: "",  
	   captchaDir: "http://concernedjoe.com/Theme/Images/Fancy Captcha/",  
	   url: "/fancycaptcha/getRand/",
	   formId: "RegistrationBoxForm",
	   text: "Verify that you are a human,<br />drag the <span>scissors</span> into the circle.",
	   items: Array("joe", "life-tile", "death-tile", "box-tile", "magnet") 
	  };	
	...
	var rand = parseInt($.ajax({ url: options.url,async: false }).responseText);

	var pic = parseInt(randomNumber());
	...
	$("#ajax-fc-circle").droppable({
		drop: function(event, ui) {
			...
			$("#" + options.formId).append("<input type=\"hidden\" style=\"display: none;\" name=\"captchaMAN\" value=\"" + rand + "\">");
		},
		tolerance: 'touch'
	});	
	};
...
$(document).ready(function() {
	$(".ajax-fc-container").captcha({formId: "myForm",captchaDir: "../../Library/Model/captcha"});
});
